...
test_encodings, test_labels_encodings = preprocess_data_test(df_test, tokenizer)
...
def test_holdout(model, loader):
    ...
    for batch in loader:
        ...
        with torch.no_grad():
            output = model.generate(input_ids.to(device), attention_mask=attention_mask.to(device), max_length=100,
                                    num_beams=4, decoder_start_token_id=model.config.decoder.pad_token_id)
        ...
        gold_summaries_holdout.extend(tokenizer.batch_decode(labels, skip_special_tokens=True))
...
rouge_scores = calculate_rouge(generated_summaries_holdout, generated_summaries_holdout)
